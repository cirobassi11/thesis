...
Tutti gli shader sono stati realizzati come shader personalizzati per la Universal Render Pipeline (URP) di Unity, scritti in HLSL all'interno della struttura ShaderLab introdotta nel \cref{sec:hlsl}.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4cm} p{4.3cm} p{4.3cm}}
        \hline
        \textbf{Sottosezione} & \textbf{Scena di riferimento} & \textbf{Aspetto analizzato}\\
        \hline
        Phong e Blinn-Phong & Scena del riflesso speculare troncato & Differenza nella componente speculare \\
        \hline
        Blinn-Phong e Cook-Torrance & Scena di confronto Blinn-Phong / Cook-Torrance & Modello empirico vs BRDF fisicamente plausibile \\
        \hline
        Variazione di metallic e roughness & Scena metallic/roughness & Effetto dei parametri del workflow PBR \\
        \hline
        Effetto delle texture & Scena dei materiali con texture & Variazione locale dei parametri \\
        \hline
        Shader non fotorealistici & Scena panoramica / scena interattiva & Differenze stilistiche rispetto ai modelli realistici \\
        \hline
    \end{tabular}
...
Una terza differenza riguarda l'effetto Fresnel, presente in Cook-Torrance tramite il termine $F$ (\cref{subsec:fresnel}) e assente in Blinn-Phong. Poiché negli shader sviluppati non è implementato l'Image-Based Lighting, il Fresnel non si manifesta come riflesso dell'ambiente, ma come aumento della riflettanza speculare agli angoli radenti. Lo si può osservare spostando il punto di vista in modo che il riflesso speculare si avvicini al bordo della sfera: in Cook-Torrance diventa più intenso, mentre in Blinn-Phong resta invariato (\cref{fig:conf-fresnel}).
...
\begin{figure}[H]
    ...
    Parametri: metallic da 0 a 1 (orizzontale), roughness da 0 a 1 (verticale).}
...
Infine, la versione della BRDF di Cook-Torrance implementata è quella per luce diretta: il fattore $k$ del termine geometrico utilizza la rimappatura della rugosità valida per le sorgenti dirette (\cref{subsec:geometry}), e non quella prevista per il contributo dell'Image-Based Lighting.
