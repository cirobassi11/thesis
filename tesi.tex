...
Gli shader dei modelli di Phong e Blinn-Phong condividono la stessa struttura, basata sul Phong shading: un vertex shader che trasforma posizione e normale dallo spazio oggetto allo spazio mondo, e un fragment shader che applica il modello di illuminazione per ogni frammento. La differenza tra i due risiede unicamente nel calcolo della componente speculare. I parametri del materiale vengono esposti nelle \texttt{Properties}, così da poter essere regolati dal pannello descritto nella \Cref{subsec:pannello}.
...
Lo shader PBR implementa la BRDF di Cook-Torrance (\Cref{sec:cook-torrance}) e permette di valutare una resa fisicamente plausibile. Espone i parametri del workflow metallic/roughness: albedo, metallic, roughness e un termine ambientale. La struttura del vertex shader è identica a quella dei modelli classici.
...
Questa variante del modello di Cook-Torrance mostra come i parametri del materiale possano variare punto per punto, leggendoli da texture anziché da valori costanti. Espone una albedo map, una metallic map, una roughness map e una normal map. Le funzioni $D$, $F$, $G$ e la struttura del fragment shader restano identiche alla versione precedente: cambiano solo la lettura dei parametri e il calcolo della normale.
...
La normal map richiede un passaggio aggiuntivo. Le normali memorizzate nella texture sono espresse in un sistema di coordinate locale alla superficie (\textit{tangent space}) e devono essere trasformate in world space per essere utilizzate nei calcoli di illuminazione. Per farlo si costruisce la matrice \textbf{TBN} dai tre vettori che descrivono l'orientamento della superficie nel mondo (la tangente $\mathbf{T}$, la bitangente $\mathbf{B}$ e la normale $\mathbf{N}$, trasformati in world space dal vertex shader) e si moltiplica la normale letta dalla texture per tale matrice, ottenendo una normale espressa nel sistema di riferimento globale:
...
\begin{lstlisting}[style=hlsl,
    ...
    label={lst:toon-frag}]
float4 frag(Varyings IN) : SV_Target {
    // ... vettori N, L, V, H (identici al modello di Blinn-Phong)
...
Ogni oggetto modificabile è dotato dello script \texttt{SelectableObject}, che ne definisce il nome e l'insieme di materiali disponibili. All'avvio lo script crea una copia di ciascun materiale, in modo che le modifiche apportate a runtime siano indipendenti per ogni oggetto e non alterino i materiali originali del progetto. Il primo materiale dell'elenco viene applicato come predefinito.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{img/project/shader_panel.jpeg}
...
Questa scena confronta il modello empirico di Blinn-Phong con il modello di Cook-Torrance, su sfere identiche e sotto la stessa illuminazione. Sono previste due configurazioni, pensate per evidenziare due differenze distinte:
...
\begin{itemize}
    ...
    \item un materiale con rugosità elevata: evidenzia lo scurimento dei bordi prodotto dal termine $G$ del PBR, assente in Blinn-Phong.
\end{itemize}
...
Questa scena dispone una griglia di sfere identiche con shader Cook-Torrance, in cui il parametro metallic varia lungo un asse e la roughness lungo l'altro. Permette di osservare in un'unica immagine come i due parametri modifichino l'aspetto del materiale.
...
Trattandosi di rendering in tempo reale, è opportuno considerare anche il costo computazionale dei diversi shader. Va premesso che, su hardware moderno e per scene semplici, le differenze di prestazioni tra i modelli sono trascurabili: il calcolo dell'illuminazione viene eseguito in parallelo dalla GPU per ogni frammento, e la complessità aggiuntiva dei modelli più elaborati incide in modo marginale sul tempo di rendering complessivo. Per questo motivo viene inserita una valutazione qualitativa del costo relativo, anziché una misurazione numerica che, in queste condizioni, risulterebbe poco significativa.
