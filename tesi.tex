...
Gli shader che implementano i modelli di illuminazione (Phong, Blinn-Phong e Cook-Torrance) condividono la stessa struttura di base: un vertex shader che trasforma posizione e normale dallo spazio oggetto allo spazio mondo, e un fragment shader che applica il modello di illuminazione per ogni frammento. A cambiare tra uno shader e l'altro sono soltanto le proprietà esposte nel blocco \texttt{Properties} e il contenuto del fragment shader; il vertex shader e le strutture \texttt{Attributes} e \texttt{Varyings} restano invariati. Le proprietà del materiale vengono esposte nelle \texttt{Properties}, così da poter essere regolate dal pannello descritto nella \cref{subsec:pannello}.
...
Lo shader PBR implementa la BRDF di Cook-Torrance (\cref{sec:cook-torrance}) e permette di valutare una resa fisicamente plausibile. Adotta la struttura comune descritta nella \cref{subsec:struttura-comune}, esponendo però le proprietà del workflow metallic/roughness (\texttt{\_Albedo}, \texttt{\_Metallic}, \texttt{\_Roughness}) e un termine ambientale (\texttt{\_AmbientK}) al posto dei coefficienti dei modelli classici.
...
La normal map richiede un passaggio aggiuntivo. Le normali memorizzate nella texture sono espresse in un sistema di coordinate locale alla superficie (\textit{tangent space}) e devono essere trasformate in world space per essere utilizzate nei calcoli di illuminazione. Per farlo si costruisce la matrice \textbf{TBN} dai tre vettori che descrivono l'orientamento della superficie nel mondo (la tangente $\mathbf{T}$, la bitangente $\mathbf{B}$ e la normale $\mathbf{N}$, trasformati in world space dal vertex shader) e si moltiplica la normale letta dalla texture per questa matrice, ottenendo una normale espressa nel sistema di riferimento globale:
...
Una terza differenza riguarda l'effetto Fresnel, presente in Cook-Torrance tramite il termine $F$ (\cref{subsec:fresnel}) e assente in Blinn-Phong. Poiché negli shader sviluppati non è implementato l'Image-Based Lighting, il Fresnel non si manifesta come riflesso dell'ambiente, ma come aumento della riflettanza speculare agli angoli radenti. Lo si può osservare spostando il punto di vista in modo che il riflesso speculare passi dal centro della sfera verso il bordo: in Cook-Torrance il riflesso diventa più intenso man mano che si avvicina al contorno, mentre in Blinn-Phong la sua intensità resta invariata (\cref{fig:conf-fresnel}).
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Effetto Fresnel su luce diretta]{Effetto Fresnel con luce diretta. In Blinn-Phong il riflesso ha la stessa intensità al centro (a) e al bordo (b). In Cook-Torrance il riflesso al bordo (d) è più intenso che al centro (c).}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/project/screenshot/texture.jpg}
