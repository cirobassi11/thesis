...
Uno shader Unity è organizzato nei seguenti blocchi: \texttt{Properties}, che espone i parametri del materiale, configurabili nell'inspector di Unity; \texttt{Shader}, il blocco esterno identificato da un nome; uno o più \texttt{SubShader}, ciascuno pensato per una specifica piattaforma; uno o più \texttt{Pass}, che rappresentano singole esecuzioni della pipeline, ognuna con il proprio vertex e fragment shader.
...
Il \textit{rim lighting} illumina i bordi dell'oggetto sfruttando lo stesso principio dell'effetto Fresnel: il prodotto $\mathbf{N} \cdot \mathbf{V}$ misura quanto la superficie è rivolta verso l'osservatore. Ai bordi, dove la superficie è quasi perpendicolare alla direzione di vista, questo valore tende a zero e il contributo del rim lighting è al massimo:
...
Fa eccezione la variante del modello di Cook-Torrance con texture, descritta nella \cref{subsec:impl-pbr-texture}, che richiede tra gli \texttt{Attributes} anche le coordinate UV (per campionare le texture) e la tangente. Dalla tangente e dalla normale, trasformate in world space dal vertex shader, si ricava la bitangente: i tre vettori, trasmessi tramite i \texttt{Varyings}, vengono utilizzati nel fragment shader per costruire la matrice \textbf{TBN}.
...
\begin{itemize}
    ...
    \item \textbf{Tessellation e displacement mapping}: l'aggiunta dello stadio di tessellation permetterebbe di suddividere dinamicamente la geometria e, combinata con il displacement mapping, di rappresentare dettagli di superficie come rilievi reali anziché simulati. A differenza del normal mapping, che altera solo le normali, questi dettagli risulterebbero visibili anche sulla forma dell'oggetto e nelle ombre proiettate.
