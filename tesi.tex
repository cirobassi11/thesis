...
\begin{itemize}
    ...
    \item il \Cref{chap:pbr} è dedicato alla teoria del Physically Based Rendering: i principi fisici alla base del modello, la BRDF di Cook-Torrance, il workflow metallic/roughness e
    un'introduzione all'Image-Based Lighting;
    ...
    programmabile;
    \item il \Cref{chap:progetto} costituisce il cuore del lavoro: raccoglie l'implementazione HLSL di tutti gli shader, la loro integrazione nel progetto Unity, le scene di test e il confronto visivo e prestazionale tra i modelli;
...
\begin{itemize}
    ...
    \item \textbf{Riflesso speculare troncato}: quando l'angolo tra $\mathbf{R}$ e $\mathbf{V}$ supera i 90°, il prodotto scalare diventa negativo e viene annullato (poiché limitato a zero). In alcuni casi questo produce un taglio netto del riflesso speculare (\Cref{fig:cutoff-confronto}).
\end{itemize}
...
Tutti gli shader sono stati realizzati come shader custom per la Universal Render Pipeline (URP) di Unity, scritti in HLSL all'interno della struttura ShaderLab introdotta nel \Cref{sec:hlsl}. 

\section{Shader implementati}
\label{sec:shader-implementati}

Sono stati implementate le tre tecniche di shading (flat, Gouraud, Phong), i tre modelli di illuminazione trattati (Phong, Blinn-Phong, Cook-Torrance), la variante di Cook-Torrance con texture e i due shader non fotorealistici (toon e hologram). La \Cref{tab:shader-riepilogo} ne riassume i parametri principali e lo scopo all'interno del confronto.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{2.8cm} p{5cm} p{5cm}}
        ...
        \hline
        PBR con texture & albedo, metallic, roughness e normal map & Mostrare la variazione locale dei parametri \\
        \hline
...
Nel \textbf{flat shading} il modello di illuminazione viene valutato nel vertex shader e il risultato è annotato con il modificatore \texttt{nointerpolation}, che impedisce alla GPU di interpolare il colore tra i vertici: l'intero triangolo riceve così lo stesso valore.
...
Nel \textbf{Phong shading} il calcolo viene spostato nel fragment shader. Il vertex shader non calcola più il colore, ma passa la normale e la posizione, che vengono interpolate dalla rasterizzazione. Il fragment shader utilizza la normale interpolata (rinormalizzata) per valutare il modello di illuminazione in ogni frammento:
...
\subsection{Struttura comune dei modelli classici}
\label{subsec:struttura-comune}

Gli shader dei modelli classici (Phong e Blinn-Phong) condividono la stessa struttura, basata sul Phong shading: un vertex shader che trasforma posizione e normale dallo spazio oggetto allo spazio mondo, e un fragment shader che applica il modello di illuminazione per ogni frammento. La differenza tra i due risiede unicamente nel calcolo della componente speculare. I parametri del materiale vengono esposti come proprietà nel blocco \texttt{Properties}, così da poter essere regolati a runtime dal pannello descritto nella \Cref{subsec:pannello}, senza modificare il codice.
...
Lo shader di Phong implementa il modello classico originale (\Cref{sec:phong}) e funge da riferimento di partenza nel confronto. Espone i coefficienti del modello (ambient, diffuse e specular), l'esponente di lucentezza (shininess) e il colore del materiale. Il \textit{fragment shader} traduce direttamente l'equazione~\eqref{eq:phong-complete}, calcolando il vettore di riflessione 
$\mathbf{R}$ e la componente speculare in funzione dell'angolo tra $\mathbf{R}$ e la direzione di vista $\mathbf{V}$.
...
Lo shader di Blinn-Phong (\Cref{sec:blinn-phong}) differisce da quello di Phong unicamente nel calcolo della componente speculare: il vettore di riflessione $\mathbf{R}$ viene sostituito dall'halfway vector $\mathbf{H}$ e l'esponente di lucentezza viene moltiplicato per quattro per ottenere un riflesso comparabile.
...
Lo shader PBR implementa la BRDF di Cook-Torrance (\Cref{sec:cook-torrance}) e permette di valutare una resa fisicamente plausibile, confrontabile con i modelli classici. Espone i parametri del workflow metallic/roughness: albedo, metallic, roughness e un termine ambientale. La struttura del vertex shader è identica a quella dei modelli classici.

I tre termini $D$, $F$ e $G$ della BRDF sono implementati come funzioni HLSL separate, richiamate nel fragment shader.
...
float D_GGX(float3 N, float3 H, float roughness) {
    float a  = pow(roughness, 2.0); // alpha = roughness^2
...
Questa variante mostra come i parametri del materiale possano variare punto per punto, leggendoli da texture anziché da valori costanti. Espone una albedo map, una metallic map, una roughness map e una normal map. Le funzioni $D$, $F$, $G$ e la struttura del fragment shader restano identiche alla versione precedente: cambiano solo la lettura dei parametri e il calcolo della normale.
...
La normal map richiede un passaggio aggiuntivo. Le normali memorizzate nella texture sono espresse in un sistema di coordinate locale alla superficie (\textit{tangent space}) e devono essere trasformate in world space per essere utilizzate nei calcoli di illuminazione. Per farlo si costruisce la matrice \textbf{TBN} dai tre vettori che descrivono l'orientamento della superficie nel mondo — la tangente $\mathbf{T}$, la bitangente $\mathbf{B}$ e la normale $\mathbf{N}$, trasformati in world space dal vertex shader — e si moltiplica per essa la normale letta dalla texture:
...
Lo shader toon (\Cref{sec:toon}) produce una resa non fotorealistica quantizzando l'illuminazione di Blinn-Phong. È composto da due pass: il primo disegna il contorno, il secondo applica lo shading stilizzato. Espone il numero di fasce, la soglia speculare e i parametri del contorno.
...
\begin{lstlisting}[style=hlsl,
    caption={Primo pass del toon shading: contorno.},
...
Lo shader hologram (\Cref{sec:hologram}) combina rim lighting, pattern a strisce e flicker, e introduce la trasparenza. La configurazione del pass abilita l'alpha blending, imposta la coda di rendering a \texttt{Transparent} e disabilita la scrittura nel depth buffer:
...
In questo modo lo stesso pannello si adatta automaticamente a qualsiasi shader: un materiale Blinn-Phong mostra i controlli per ambient, diffuse, specular e shininess, mentre un materiale PBR mostra metallic e roughness, senza bisogno di codice specifico per ciascun caso.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{img/project/shader_panel.jpeg}
...
Lo script è presente in tutte le scene, così che il passaggio sia sempre disponibile. Le scene devono essere registrate nell'ordine corrispondente nelle impostazioni di build di Unity.
...
L'illuminazione è affidata a un'unica luce direzionale. La scelta di una sola sorgente è deliberata: avere una luce sola, di cui si controllano direzione, intensità e colore, permette di isolare il comportamento di ciascun modello senza che contributi multipli si sovrappongano, rendendo il confronto più leggibile. La luce direzionale, inoltre, modella una sorgente infinitamente lontana con raggi paralleli, l'approssimazione più adatta allo studio dei modelli trattati. La scena è completata da uno skybox e da un piano che funge da pavimento.

Gli oggetti selezionabili sono organizzati in modo che ciascuno parta con un materiale predefinito coerente con la sua natura, ma possa essere riconfigurato con uno qualsiasi degli shader implementati.
...
Questa scena dispone una fila di sfere identiche, ciascuna con uno degli shader implementati (Phong, Blinn-Phong, Cook-Torrance, toon e hologram), sotto la stessa luce e con lo stesso punto di vista. Permette di cogliere a colpo d'occhio le differenze complessive tra i modelli, prima dei confronti mirati delle scene successive.
...
Questa scena è dedicata al confronto tra Phong e Blinn-Phong in presenza del riflesso speculare troncato (\Cref{subsec:half-vector}). Contiene sfere e piani illuminati da una luce radente, quasi parallela alla superficie, con un valore di shininess molto basso: in queste condizioni il taglio netto del riflesso di Phong diventa evidente, mentre Blinn-Phong mantiene una transizione continua. Il piano, in particolare, rende il fenomeno più leggibile rispetto alla sola sfera. L'analisi dei risultati è riportata nella \Cref{sec:confronto-visivo}.

\subsection{Scena di confronto Blinn-Phong / Cook-Torrance}
\label{subsec:scena-blinn-pbr}

Questa scena confronta il modello empirico di Blinn-Phong con il modello fisicamente plausibile di Cook-Torrance, su sfere identiche e sotto la stessa illuminazione. Sono previste due configurazioni, pensate per evidenziare due differenze distinte:

\begin{itemize}
    \item un \textbf{materiale metallico} (oro): mette in luce come Blinn-Phong produca un riflesso speculare bianco, mentre Cook-Torrance lo colori in base all'albedo del metallo;
    \item un \textbf{materiale con rugosità elevata}: evidenzia lo scurimento dei bordi prodotto dal termine $G$ del PBR, assente in Blinn-Phong.
\end{itemize}
...
Questa scena dispone una griglia di sfere identiche con shader Cook-Torrance, in cui il parametro metallic varia lungo un asse e la roughness lungo l'altro. Permette di osservare in un'unica immagine come i due parametri governino l'aspetto del materiale, dalla superficie dielettrica e liscia a quella metallica e rugosa.

Per evitare di configurare manualmente le decine di sfere della griglia, queste vengono generate a runtime da uno script che ne imposta i parametri in base alla posizione:
...
for (int x = 0; x < size; x++) {
    for (int y = 0; y < size; y++) {
        ...
        m.SetFloat("_Roughness", max(y / (float)(size - 1), 0.05));
    }
}
\end{lstlisting}
\end{minipage}
...
Questa scena raccoglie alcune sfere con shader Cook-Torrance configurate con set di texture diversi (albedo, metallic, roughness e normal map), corrispondenti a materiali reali come legno, metallo e pietra. A differenza delle altre scene, in cui i parametri sono costanti e variano solo tra una sfera e l'altra, qui ogni sfera mostra la variazione locale dei parametri sulla propria superficie, resa possibile dalle texture. La scena serve a evidenziare il contributo delle texture map alla resa di materiali complessi.
...
In condizioni di illuminazione ordinaria, su una superficie curva, i modelli di Phong e Blinn-Phong producono risultati molto simili, a patto di equiparare i parametri (in particolare usando per Blinn-Phong un esponente di shininess maggiore, come discusso nella \Cref{subsec:half-vector}). La differenza nella forma del riflesso speculare è in questo caso sottile e difficilmente percepibile.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Confronto tra Phong e Blinn-Phong]{Confronto tra Phong e Blinn-Phong con parametri equiparati. In condizioni ordinarie i due modelli sono quasi indistinguibili.}
    \label{fig:conf-phong-blinn}
\end{figure}

La differenza diventa invece netta in presenza di luce radente e shininess bassa, condizione riprodotta nella scena del riflesso speculare troncato. Nel modello di Phong il riflesso si interrompe bruscamente con un bordo netto, dove l'angolo tra il vettore di riflessione $\mathbf{R}$ e la direzione di vista $\mathbf{V}$ supera i 90° e il prodotto scalare viene azzerato. In Blinn-Phong, dove la speculare dipende dall'angolo tra $\mathbf{N}$ e l'halfway vector $\mathbf{H}$, questa condizione non si verifica e il riflesso sfuma in modo continuo.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Riflesso speculare troncato]{Riflesso speculare su una sfera con shininess molto bassa. In Phong il riflesso presenta un taglio netto, assente in Blinn-Phong.}
...
Il confronto tra Blinn-Phong e Cook-Torrance evidenzia il passaggio da un modello empirico a uno fisicamente fondato. La differenza più immediata si osserva sui materiali metallici. In Blinn-Phong il riflesso speculare assume sempre il colore della luce, indipendentemente dal materiale: una sfera dorata presenta quindi un riflesso bianco, percettivamente innaturale. In Cook-Torrance, invece, per i materiali metallici la riflettanza base $F_0$ coincide con l'albedo (\Cref{subsec:fresnel}), e il riflesso assume il colore caratteristico del metallo.
...
Una seconda differenza emerge con materiali rugosi. Il termine $G$ di Cook-Torrance modella l'auto-ombreggiatura tra le microfacce (\Cref{subsec:geometry}), attenuando la luce nelle zone osservate o illuminate ad angoli radenti. Il risultato è uno scurimento dei bordi che aumenta con la rugosità, assente in Blinn-Phong, dove la superficie appare uniformemente illuminata.
...
La griglia di sfere generata nella scena dedicata mostra l'effetto combinato dei due parametri del workflow metallic/roughness. Lungo l'asse del metallic, all'aumentare del valore la componente diffusa scompare progressivamente e il riflesso speculare assume il colore dell'albedo: la sfera passa da un aspetto plastico a uno metallico. Lungo l'asse della roughness, all'aumentare del valore il riflesso si allarga e perde intensità, passando da una superficie a specchio a una opaca.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{img/project/screenshot/metallic-roughness.jpg}
...
L'uso delle texture non modifica il modello di illuminazione, ma permette di variarne i parametri punto per punto. Rispetto a un materiale con valori costanti, la versione con texture mostra dettagli e variazioni locali (zone più o meno metalliche, rugosità non uniforme, rilievi simulati dalla normal map) che un singolo valore costante non può rappresentare.
...
Toon e hologram non hanno l'obiettivo di simulare la realtà, ma di mostrare la flessibilità della pipeline programmabile. Il toon shading, pur basandosi sull'illuminazione di Blinn-Phong, ne quantizza il risultato in fasce nette, producendo un aspetto da disegno animato lontano dal fotorealismo dei modelli precedenti. L'effetto ologramma combina invece rim lighting, trasparenza e animazione temporale per ottenere un risultato puramente stilistico, non riconducibile ad alcun modello fisico. Entrambi dimostrano che la pipeline non è vincolata alla resa realistica: lo stesso meccanismo usato per Phong, Blinn-Phong e Cook-Torrance può produrre qualsiasi modello di resa definito dallo sviluppatore.

\section{Considerazioni sul costo computazionale}
\label{sec:costo}

Trattandosi di rendering in tempo reale, è opportuno considerare anche il costo computazionale dei diversi shader. Va premesso che, su hardware moderno e per scene semplici come quelle realizzate, le differenze di prestazioni tra i modelli sono trascurabili: il calcolo dell'illuminazione viene eseguito in parallelo dalla GPU per ogni frammento, e la complessità aggiuntiva dei modelli più elaborati incide in modo marginale sul tempo di rendering complessivo. Per questo motivo si propone una valutazione qualitativa del costo relativo, anziché una misurazione numerica che, in queste condizioni, risulterebbe poco significativa.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{3cm} p{2.2cm} p{7cm}}
        ...
        \hline
        Phong & basso & Poche operazioni vettoriali; richiede il calcolo del vettore di riflessione. \\
        \hline
        ...
        \hline
        PBR con texture & medio-alto & Aggiunge il campionamento di più texture e la trasformazione della normale tramite la matrice TBN. \\
        \hline
        Toon & medio & Richiede un secondo pass di rendering per il contorno. \\
        \hline
        Hologram & medio & Utilizza la trasparenza con alpha blending ed effetti animati nel tempo. \\
        \hline
    \end{tabular}
...
In sintesi, i modelli classici sono i più economici, il PBR ha un costo intermedio dovuto al calcolo della BRDF, e la variante con texture aggiunge il costo dei campionamenti. Gli shader non fotorealistici introducono un costo aggiuntivo non per la complessità del modello di illuminazione, ma per le tecniche che impiegano: il secondo pass nel toon e il blending nell'hologram. In tutti i casi, la scelta del modello in un'applicazione reale dipende più dalla qualità visiva desiderata che dal costo computazionale, che diventa rilevante solo in scene molto complesse o con un elevato numero di sorgenti luminose.

\section{Limiti dell'implementazione}
\label{sec:limiti-impl}

L'implementazione realizzata è volutamente semplificata, per concentrare l'attenzione sui modelli di illuminazione e mantenere il codice leggibile. Presenta quindi alcuni limiti, che è opportuno esplicitare.

Il principale riguarda la gestione della luce. Tutti gli shader considerano un'unica sorgente direzionale, ottenuta tramite \texttt{GetMainLight}: non è previsto il supporto a sorgenti multiple, né a luci puntuali o spot, che richiederebbero un ciclo sui contributi di ciascuna sorgente. Questa scelta semplifica il confronto, ma limita gli shader a scene con una sola luce.
...
Gli shader non gestiscono inoltre le ombre proiettate, che richiederebbero un pass aggiuntivo di shadow mapping, né la trasparenza in forma generale: l'unico shader trasparente è l'effetto ologramma, configurato in modo specifico.

Infine, la versione della BRDF di Cook-Torrance implementata è quella per luce diretta: il fattore $k$ del termine geometrico utilizza la rimappatura della rugosità valida per le sorgenti dirette (\Cref{subsec:geometry}), e non quella prevista per il contributo dell'Image-Based Lighting, coerentemente con il fatto che quest'ultimo non è stato implementato.
...
Questo elaborato ha presentato i principali modelli di illuminazione utilizzati nella grafica in tempo reale, dai modelli empirici classici fino al Physically Based Rendering, accompagnando la trattazione teorica con l'implementazione concreta di tutti i modelli in HLSL all'interno del motore Unity. La parte teorica e quella pratica sono state tenute distinte: i \Cref{chap:classici,chap:pbr,chap:custom} hanno presentato i fondamenti dei modelli, mentre il \Cref{chap:progetto} ne ha raccolto l'implementazione, l'integrazione in Unity e il confronto sistematico.

\section{Riepilogo dei risultati}

Sono stati implementati e confrontati i tre principali modelli di illuminazione (Phong, Blinn-Phong e Cook-Torrance), le tre tecniche di shading (flat, Gouraud e Phong shading) e due shader non fotorealistici (toon e ologramma), ciascuno realizzato come shader custom per la Universal Render Pipeline di Unity. Gli shader sono stati integrati in un progetto interattivo che, insieme a un insieme di scene di test dedicate, ha permesso di confrontarli direttamente nelle stesse condizioni di illuminazione.
...
Il lavoro si è concentrato sul confronto dei modelli di illuminazione in condizioni controllate e semplici, accettando alcune semplificazioni discusse in dettaglio nella \Cref{sec:limiti-impl}. La principale è la gestione della luce indiretta, approssimata con un termine ambientale costante anziché tramite Image-Based Lighting: questa scelta, pur sufficiente a dimostrare il funzionamento dei modelli, ne limita la resa, in particolare per i materiali metallici. L'implementazione si è inoltre limitata a una singola sorgente direzionale, senza ombre proiettate né sorgenti multiple.

Queste semplificazioni, coerenti con l'obiettivo didattico e comparativo del lavoro, lasciano spazio a diversi sviluppi.

\section{Possibili sviluppi futuri}

Il lavoro potrebbe essere esteso in diverse direzioni:
\begin{itemize}
    \item \textbf{Image-Based Lighting}: l'implementazione completa dell'IBL, con irradiance map e cubemap filtrate per la componente speculare, migliorerebbe sensibilmente la resa dei materiali, in particolare dei metalli, e renderebbe pienamente visibile l'effetto Fresnel.
    ...
    \item \textbf{Modelli avanzati}: BRDF più sofisticate permetterebbero di rappresentare una gamma più ampia di materiali, come quelli anisotropi o traslucidi (\textit{subsurface scattering}).
