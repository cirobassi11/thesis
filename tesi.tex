...
\end{itemize}
Le texture fungono quindi da semplice contenitore per i dati: il modello matematico sottostante rimane invariato, indipendentemente dal fatto che i parametri provengano da una texture o da valori costanti. Nella versione base dello shader Cook-Torrance i parametri del materiale vengono forniti come valori costanti, così da isolare il comportamento del modello di illuminazione. Nel \cref{chap:progetto} viene inoltre presentata una variante con texture, che consente di variare albedo, metallic, roughness e normale punto per punto sulla superficie.
...
Nel \textbf{flat shading} il modello di illuminazione viene valutato nel vertex shader e il risultato è annotato con il modificatore \texttt{nointerpolation}, che impedisce alla GPU di interpolare il colore tra i vertici: ogni frammento del triangolo riceve così lo stesso valore.
...
Fa eccezione la variante del modello di Cook-Torrance con normal map (\cref{subsec:impl-pbr-texture}): per campionare le texture e perturbare la normale, richiede anche le coordinate UV e il vettore tangente tra gli \texttt{Attributes}, e di conseguenza tangente e bitangente (necessarie per costruire la matrice TBN) tra i \texttt{Varyings}.
...
La griglia di sfere generata nella scena descritta nella sezione \cref{subsec:scena-griglia} mostra l'effetto combinato dei due parametri del workflow metallic/roughness. Lungo l'asse del metallic, all'aumentare del valore la componente diffusa scompare progressivamente e il riflesso speculare assume il colore del materiale: la sfera passa da un aspetto plastico a uno metallico. Lungo l'asse della roughness, all'aumentare del valore il riflesso si allarga e perde intensità, passando da una superficie a specchio a una opaca, come mostrato in figura \cref{fig:conf-griglia}.
...
Questo confronto è stato ottenuto nella scena panoramica descritta nella sezione \cref{subsec:scena-panoramica}, nella quale gli shader implementati sono applicati a oggetti identici e osservati nelle stesse condizioni di illuminazione.
...
\begin{itemize}
    ...
    \item \textbf{Modelli avanzati}: BRDF più sofisticate permetterebbero di rappresentare una gamma più ampia di materiali, come quelli anisotropi o traslucidi.
