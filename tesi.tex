...
\begin{figure}[H]
    ...
    \caption[Vettori coinvolti nel modello di Phong]{Vettori coinvolti nel modello di Phong. Immagine adattata da ~\cite{wiki_blinnphong}.}
...
\begin{figure}[H]
    ...
    \caption[Vettori coinvolti nel modello di Blinn-Phong]{Vettori coinvolti nel modello di Blinn-Phong. Immagine adattata da ~\cite{wiki_blinnphong}.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Confronto tra i modelli di Phong e Blinn-Phong]{Confronto tra i modelli di Phong e Blinn-Phong con parametri identici.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Confronto tra i riflessi speculari con luce radente]{Riflesso speculare su un piano con luce radente e un valore di shininess basso. Nel modello di Phong il riflesso presenta un taglio netto, assente in Blinn-Phong.}
...
\begin{figure}[H]
    ...
    \end{tabular}
    \caption[Effetti di masking e shadowing]{Nel masking (a) la microfaccia, pur orientata correttamente, non è visibile all'osservatore perché coperta da una microfaccia adiacente; nello shadowing (b) la luce non raggiunge la microfaccia perché bloccata da una vicina. Immagini tratte da \cite{pharr2023}.}
...
\begin{figure}[H]
    ...
    \caption[Confronto tra luce diretta e indiretta]{Confronto tra luce diretta e luce indiretta: la luce diretta raggiunge la superficie direttamente dalla sorgente, mentre la luce indiretta è il risultato dei rimbalzi successivi sulle superfici circostanti.}
...
\begin{figure}[H]
    ...
    \caption[Cubemap]{Una \textit{cubemap} in formato a croce: le sei facce del cubo vengono "srotolate" in un'unica immagine e rappresentano l'ambiente circostante a 360°.}
    \label{fig:cubemap}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Confronto tra i modelli di Blinn-Phong e di Cook-Torrance]{Confronto tra i modelli di Blinn-Phong e di Cook-Torrance nelle stesse condizioni di illuminazione.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.19\textwidth}
        ...
        \label{fig:metal-1}
    \end{subfigure}
    \caption[Effetto del parametro metallic]{Effetto del parametro metallic (roughness = 0.3). Al crescere di metallic la componente diffusa scompare e il riflesso speculare assume il colore dell'albedo.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Confronto con rugosità elevata]{Confronto con rugosità elevata. In Cook-Torrance i bordi risultano più scuri per effetto del termine G (auto-ombreggiatura delle microfacce), assente in Blinn-Phong.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \label{fig:pbr-tex}
    \end{subfigure}
    \caption[Confronto tra valori costanti e texture maps]{Il modello di Cook-Torrance con valori costanti e con texture maps. Le texture permettono di variare i parametri del materiale punto per punto.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \label{fig:toon-8}
    \end{subfigure}
    \caption[Effetto del numero di fasce nel toon shading]{Effetto del numero di fasce nel toon shading. Con poche fasce l'aspetto è più stilizzato, con più fasce la transizione è più graduale.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Confronto tra Blinn-Phong e toon shading]{Confronto tra Blinn-Phong e toon shading sullo stesso oggetto.}
...
Per verificare nella pratica le differenze tra i modelli di illuminazione analizzati e permettere un confronto interattivo, è stato realizzato un progetto dimostrativo in Unity. L'applicazione consente di esplorare una scena in prima persona, selezionare i singoli oggetti e modificarne lo shader e i relativi parametri, osservando l'effetto delle modifiche in tempo reale.
...
\begin{figure}[H]
    ...
    \\[2em]
    \includegraphics[width=1.0\textwidth]{img/project/scene2.jpeg}
    \caption[Panoramica della scena dimostrativa]{Panoramica della scena dimostrativa da due angolazioni diverse.}
    \label{fig:scena}
\end{figure}
...
rotX -= mouseY;
rotX = Mathf.Clamp(rotX, -90f, 90f); // Rotazione verticale limitata
transform.localRotation = Quaternion.Euler(rotX, 0f, 0f); // Rotazione verticale
transform.parent.Rotate(Vector3.up * mouseX); // Rotazione orizzontale
...
Ogni oggetto modificabile è dotato dello script \texttt{SelectableObject}, che ne definisce il nome e l'insieme di materiali disponibili e fornisce un metodo per applicarli. All'avvio, lo script crea una copia di ciascun materiale, in modo che le modifiche apportate durante l'esecuzione siano indipendenti per ogni oggetto e non alterino i materiali originali. Il primo materiale dell'elenco viene applicato come predefinito.
...
\begin{minipage}{\textwidth}
\begin{lstlisting}[style=hlsl,
    caption={Creazione delle istanze dei materiali e applicazione in tutti i Renderer dell'oggetto.},
    label={lst:selectable}]
void Awake() {
    // Renderers dell'oggetto e dei figli
...
\begin{figure}[H]
    ...
    \caption[Il pannello di selezione dello shader]{Il pannello di selezione dello shader aperto su un oggetto selezionato.}
    \label{fig:pannello}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Lo stesso oggetto con i sei shader implementati]{Lo stesso oggetto con i sei shader implementati, nella stessa scena e con la stessa illuminazione.}
...
Il progetto è stato infine esportato come applicazione eseguibile tramite il sistema di build di Unity, generando una versione indipendente dall'editor. In questo modo l'applicazione può essere eseguita direttamente, senza necessità di aprire il progetto in Unity, ed è utilizzabile per un confronto interattivo dei modelli implementati.
...
L'implementazione presentata si concentra sulla luce diretta proveniente da una singola sorgente direzionale, escludendo il supporto a luci multiple e ombre proiettate. Il contributo della luce indiretta, normalmente gestito tramite Image-Based Lighting, è stato approssimato con un termine ambientale costante. Questa semplificazione, pur sufficiente a dimostrare il funzionamento del modello, limita la resa visiva dei materiali, in particolare dei metalli, che senza riflessi ambientali appaiono più scuri di quanto sarebbero in un ambiente reale.

\section{Possibili sviluppi futuri}
Il lavoro potrebbe essere esteso in diverse direzioni:
\begin{itemize}
    \item \textbf{Luci multiple e ombre}: il supporto a più sorgenti luminose e alle ombre proiettate renderebbe gli shader utilizzabili anche in scene più complesse e realistiche.
