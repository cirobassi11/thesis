...
\begin{table}[H]
    ...
    \begin{tabular}{l c c}
        ...
        \hline
    \end{tabular}
    \caption{Valori di $F_0$ per alcuni materiali comuni. Sopra la linea i dielettrici, con valori bassi e acromatici; sotto i metalli, con valori alti e colorati.}
    \label{tab:f0-valori}
\end{table}
...
Tutti gli shader sono stati realizzati come shader custom per la Universal Render Pipeline (URP) di Unity, scritti in HLSL all'interno della struttura ShaderLab introdotta nel \Cref{sec:hlsl}. Il capitolo si apre con le implementazioni, prosegue con la descrizione del progetto e delle scene di test, e si conclude con il confronto tra i modelli e alcune considerazioni sul costo computazionale.

\section{Shader implementati}
\label{sec:shader-implementati}

Sono stati implementate le tre tecniche di shading (flat, Gouraud, Phong shading), i tre modelli di illuminazione trattati (Phong, Blinn-Phong, Cook-Torrance), la variante di Cook-Torrance con texture e i due shader non fotorealistici (toon e hologram). La \Cref{tab:shader-riepilogo} ne riassume i parametri principali e lo scopo all'interno del confronto.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{2.8cm} p{5cm} p{5cm}}
        ...
        \hline
    \end{tabular}
    \caption[Riepilogo degli shader implementati]{Riepilogo degli shader implementati, con i parametri principali e lo scopo all'interno del confronto. Flat e Gouraud shading non compaiono come modelli a sé ma come tecniche di shading (\Cref{subsec:impl-shading-tecniche}), applicate al modello di Blinn-Phong.}
...
Le tre tecniche di shading (\Cref{sec:shading}) si distinguono per il punto in cui viene valutato il modello di illuminazione e per i dati scambiati tra vertex e fragment shader. La tecnica è indipendente dal modello di illuminazione adottato: per questo motivo, nei frammenti di codice seguenti, il calcolo dell'illuminazione è lasciato come segnaposto.
...
Tutti gli shader presentati nel resto del capitolo utilizzano il Phong shading, poiché è l'unico che produce riflessi speculari accurati indipendentemente dalla densità della mesh.

\subsection{Struttura comune dei modelli classici}
\label{subsec:struttura-comune}

Gli shader dei modelli classici (Phong e Blinn-Phong) condividono la stessa struttura, basata sul Phong shading: un vertex shader che trasforma posizione e normale dallo spazio oggetto allo spazio mondo, e un fragment shader che applica il modello di illuminazione per ogni frammento. La differenza tra i due risiede unicamente nel calcolo della componente speculare.

I parametri del materiale vengono esposti come proprietà nel blocco \texttt{Properties}, così da poter essere regolati a runtime dal pannello descritto nella \Cref{sec:pannello}, senza modificare il codice.
...
Lo shader di Phong implementa il modello classico originale (\Cref{sec:phong}) e funge da riferimento di partenza nel confronto. Espone i quattro coefficienti del modello (ambient, diffuse, specular e shininess) oltre al colore del materiale. Il fragment shader traduce direttamente l'equazione~\eqref{eq:phong-complete}, calcolando il vettore di riflessione $\mathbf{R}$ e la componente speculare in funzione dell'angolo tra $\mathbf{R}$ e la direzione di vista.
...
Lo shader di Blinn-Phong (\Cref{sec:blinn-phong}) differisce da quello di Phong unicamente nel calcolo della componente speculare: il vettore di riflessione $\mathbf{R}$ è sostituito dall'halfway vector $\mathbf{H}$, e l'esponente di shininess viene moltiplicato per quattro per ottenere un riflesso di dimensione comparabile. Permette di confrontare direttamente le due formulazioni della speculare, incluso il comportamento con luce radente.
