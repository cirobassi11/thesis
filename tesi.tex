...
L'applicazione consente di esplorare una scena in prima persona, selezionare i singoli oggetti e modificarne in tempo reale lo shader e i relativi parametri, osservando l'effetto delle
modifiche.

\section{Struttura della scena}
\label{sec:scena}

La scena contiene diversi oggetti disposti in un ambiente esplorabile, ciascuno con un proprio insieme di materiali predefiniti. Sono presenti sia primitive geometriche (sfere, cubi, cilindri) sia modelli più complessi importati in formato FBX, in modo da mostrare il comportamento degli shader su geometrie di diversa natura — superfici curve, spigoli netti e mesh organiche.
...
Ogni oggetto modificabile è dotato dello script \texttt{SelectableObject}, che ne definisce il nome e l'insieme di materiali disponibili. All'avvio, lo script crea una copia (\textit{instance}) di ciascun materiale, in modo che le modifiche apportate a runtime siano indipendenti per ogni oggetto e non alterino i materiali originali del progetto. Il primo materiale dell'elenco viene applicato come predefinito.
...
Sono stati implementati e confrontati diversi modelli di illuminazione: Phong, Blinn-Phong e Cook-Torrance, ciascuno realizzato come shader custom per la Universal Render Pipeline. Il confronto ha evidenziato come il passaggio dai modelli classici al PBR non sia una semplice evoluzione incrementale, ma un cambio
di approccio: i modelli classici approssimano l'aspetto della luce con formule empiriche i cui parametri vengono regolati manualmente, mentre il PBR si fonda su principi fisici che garantiscono per costruzione la coerenza visiva dei materiali.
...
Sono stati inoltre realizzati due shader personalizzati (toon shading ed effetto ologramma) che dimostrano come la pipeline programmabile non sia limitata al fotorealismo ma permetta di implementare qualsiasi modello di illuminazione o effetto visivo.
...
Gli shader non gestiscono inoltre ombre proiettate, luci multiple o trasparenze complesse (ad eccezione dell'effetto ologramma), funzionalità che in un contesto produttivo sarebbero necessarie.
...
\begin{itemize}
    \item \textbf{Image-Based Lighting}: l'implementazione completa dell'IBL, con irradiance map e cubemap filtrate per la componente speculare, migliorerebbe significativamente la resa dei materiali, in particolare dei metalli.
    ...
    \item \textbf{Modelli avanzati}: BRDF più sofisticate, come quelle che tengono conto dell'anisotropia del materiale o della trasmissione della luce attraverso superfici sottili (\textit{subsurface scattering}), permetterebbero di rappresentare una gamma più ampia di materiali.
\end{itemize}
