...
Per descrivere come una superficie riflette la luce si utilizza una funzione chiamata \textit{BRDF} (\textit{Bidirectional Reflectance Distribution Function}). Essa definisce, data una direzione di illuminazione L e una direzione di osservazione V, la frazione di luce incidente che viene riflessa dalla superficie verso l’osservatore.
...
Il termine $F$ descrive l'effetto Fresnel: la quantità di luce riflessa da una superficie varia in funzione dell'angolo compreso tra la direzione di osservazione $\mathbf{V}$ e la normale $\mathbf{N}$. In particolare, la riflettanza aumenta quanto più la superficie viene osservata di taglio, cioè quando l’angolo tra la direzione di vista V e la normale N diventa più ampio. Questo fenomeno è facilmente osservabile in natura: guardando uno specchio d’acqua dall’alto è spesso possibile vedere il fondo, mentre osservandolo in lontananza prevale il riflesso del paesaggio circostante.
