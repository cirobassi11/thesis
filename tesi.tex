...
\lstdefinestyle{hlsl}{
    ...
    morekeywords={float2, float3, float4, float3x3, half, half3, half4,
              ...
              TEXTURE2D, SAMPLER, SAMPLE_TEXTURE2D, Vector3, foreach, Material, Ray, RaycastHit, var},
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{img/project/scene.jpg}
    \caption{Panoramica della scena dimostrativa.}
    \label{fig:scena}
\end{figure}
...
La navigazione nella scena avviene in prima persona: il movimento è controllato dai tasti WASD e l'orientamento della visuale dal mouse. Il controllo è implementato nello script \texttt{FirstPersonCamera}, applicato alla camera. La rotazione verticale (\textit{pitch}), limitata a $\pm 90^\circ$ per evitare ribaltamenti, viene applicata direttamente alla camera, mentre la rotazione orizzontale (\textit{yaw}) viene applicata all'oggetto genitore. Il movimento utilizza un \texttt{CharacterController}, che gestisce automaticamente le collisioni con la geometria della scena.
...
rotX -= mouseY;
// Rotazione verticale limitata
rotX = Mathf.Clamp(rotX, -90f, 90f);
// Rotazione verticale applicata alla camera
transform.localRotation = Quaternion.Euler(rotX, 0f, 0f);
// Rotazione orizzontale applicata al player
...
Poiché i modelli importati possono essere composti da più oggetti annidati, lo script raccoglie tutti i \texttt{Renderer} (il componente Unity responsabile del disegno di una mesh a schermo) presenti nell'oggetto e nei suoi figli applicando il materiale a ciascuno di essi.
...
La selezione di un oggetto avviene tramite \textit{raycasting}: al click del mouse viene lanciato un raggio dal centro dello schermo, e il primo oggetto colpito dotato di \texttt{SelectableObject} viene selezionato. Un mirino al centro dello schermo indica il punto di mira.

\begin{minipage}{\textwidth}
\begin{lstlisting}[style=hlsl,
    caption={Selezione di un oggetto tramite raycast.},
    label={lst:raycast}]
// Raggio dal centro dello schermo
...
\begin{minipage}{\textwidth}
\begin{lstlisting}[style=hlsl,
    caption={Esempio di generazione di un controllo del pannello.},
    label={lst:panel}]
...
In questo modo lo stesso pannello si adatta automaticamente a qualsiasi shader: un materiale Blinn-Phong mostrerà i controlli per ambient, diffuse, specular e shininess, mentre un materiale PBR mostrerà metallic e roughness.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/project/shader_panel.jpeg}
    \caption{Il pannello di controllo aperto su un oggetto selezionato.}
    \label{fig:pannello}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Lo stesso oggetto con i sei shader implementati, nella stessa scena e con la stessa illuminazione.}
    \label{fig:obj-confronto}
\end{figure}

\section{Build dell'applicazione}
\label{sec:build}

Il progetto è stato infine esportato come applicazione eseguibile tramite il sistema di build di Unity, generando una versione indipendente dall'editor. In questo modo la demo può essere eseguita direttamente, senza necessità di aprire il progetto in Unity, ed è utilizzabile per un confronto interattivo dei modelli implementati.
